...
public class NaverController {
	...
	// 한글 data가 있으면 produces = "text/html; charset=UTF-8" 
	@ResponseBody
	...
	public List<NBookVO> book(@PathVariable(name ="title") String title) {
		 ...
		 * naver 의 API(대부분의 open AIP)들은 요청을 할때 GET method 방식을 사용해야 하는데
		 * 그래서 getFor**() 함수를 사용해야 한다.
		 * 하지만 getFor**() 함수는 Http 프로토콜의 Header 에 값을 담아서 보낼수 없다.
		 * 여기에서는 getFor**() 대신에 exchange() 함수를 사용한다.
		 * exchange() 함수는 전송 method 를 지정해야 하고, 여기에는 header 값을 담아서
		 * 보낼 수 있다. 
		 */
		
		ResponseEntity<NBooks> result = restTemplate
				.exchange(naver_book_url, HttpMethod.GET, params, NBooks.class);
