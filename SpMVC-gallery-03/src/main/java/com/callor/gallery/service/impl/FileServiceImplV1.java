...
public class FileServiceImplV1 implements FileService{
	 ...
	 * 상속을 해주려는 클래스와 상속을 받는 클래스에서 공통으로
	...
	public String fileUp(MultipartFile file) throws Exception {
		...
		if(!path.exists()) {
			 ...
			 * 폴더를 생성하는 method
			 * mkdir() : 한개의 폴더만 생성하기 만약 app/upload 폴더를 생성할 경우
			 * 		app 폴더가 없으면 오류가 발생
		...
		// 파일 원본이름앞에 uuid를 붙여 업로드할 새로운 파일이름 생성
	...
	public List<ImageVO> filesUp(MultipartHttpServletRequest files) throws Exception {
		...
		/*
		 * view의 form input[type="file"] tag 에 name( = files) 속성을 통하여
		 * 파일 리스트 추출하기
		 */
		...
		for(MultipartFile file : fileList) {
			...
			ImageVO vo = ImageVO.builder()
					...
					.i_origin_image(originName).build();
			resultFiles.add(vo);
		}
