...
public class AuthUtiles {
	...
	/*
	 * PasswordEncoder 는 비밀번호 등을 일방통행(단방향) 으로
	 * 암호화를 하기 위한 도구이다
	 * 이 클래스 type encoder 객체를 통하여 회원가입을 할때 
	 * 비밀번호를 암호화 하고 로그인을 할때 다시 한번 암호문과 비교하는 
	 * method 를 사용하게 된다.
	 ...
	 * 이런 상황이 되면 Spring Project 의 원칙은 객체를 SingleTone(한개만) 으로 만들고
	 ...
	 * 
	 * 현재 AuthUtiles 클래스는 @Component 가 선언되어 있고,
