 ...
 * 사용자가 username, password 를 입력하여 login 을 시도하면
 * 정상적인 사용자인지 확인을 하고
...
//		String password = (String)authentication.getCredentials();
		...
		if(username == null || username.isBlank() || !username.equals("n96js")) {
			// username이 null이거나 빈칸이거나 n96js 가 아니면
			// 익셉션을 강제로 발생
			
			/*
			 * 어떤 method 가 중첩하여 호출이 될때 내부에 깊이 포함된 method 에서
			 * 바깥쪽의 method 에 메시지를 전달 하고 싶을때
			 * 강제로(일부로) exception 을 발생하여 전달하는 방법
			 */
			throw new UsernameNotFoundException("USERNAME 없음");
		}
		if(password == null || password.isBlank() || !password.equals("12345")) {
			// password이 null이거나 빈칸이거나 12345 가 아니면
			// 익셉션을 강제로 발생
