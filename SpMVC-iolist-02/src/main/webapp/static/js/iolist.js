document.addEventListener("DOMContentLoaded", () => {
  ...
  const input_price = document.querySelector("input[name='io_price']");
  const input_quan = document.querySelector("input[name='io_quan']");
  const input_total = document.querySelector("input[name='io_total']");

  const total_calc = () => {
    if (input_total) {
      input_total.value = Number(input_price?.value) * Number(input_quan?.value);
    }
  };
