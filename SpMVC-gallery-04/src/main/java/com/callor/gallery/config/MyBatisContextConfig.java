...
public class MyBatisContextConfig {
	...
	public PlatformTransactionManager transactionManager() {
		...
		/*
		 * manager 객체는 PlatformTransactionManager 인터페이스를 사용하여 선언하였다.
		 * 이 인터페이스는 다양한 Transaction 을 실행하기 위한 설계도 이다.
		 * DataSourceTransactionManager 는 PlatForm...Manager 인터페이스를 사용하여
		 * 구현된 구현체 클래스이다.
		 ...
		 * 결국 Platform... 에는 없지만, DataSource... 에만 있는 
		 * setDataSoruce() method 를 사용해야 한다.
		 * 이럴때 Platform... 에는 없지만 DataSource...Manager 에만 있는 setDataSource()
		 * method 를 사용하기 위해서는 DataSource...Manager type 으로
		 * Castion(강제형변환)을 수행해야 한다.
		 */
		// 다형성
		// DataSourceTransactionManager 에 있는 method 를 사용하기 위해서 캐스팅을 해줌
