...
public class AuthProviderImpl implements AuthenticationProvider{
	...
	/*
	 * @Autowired
	 * 기본 생성자를 통하여 UserDao type 의 userDao 객체를 주입받아라 라는 선언
	 ...
	 * 메모리가 낭비되는 현상(Memory Leak, 메모리 누수)이 있을 수 있다.
	 */
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		log.debug("로그인 한 사용자 정보 {}, {}", username, password);
		...
		if(userVO == null) {
			 ...
			 * 코드의 진행을 중단하고, method 를 deep 호출한 곳 까지 되돌리기
			 * 결국 Front 까지 메시지를 전달하기 위한 방법
			 * exception 이 발생하면 실행 전으로 돌려버림
			 */
			throw new UsernameNotFoundException(String.format("USERNAME 확인 : %s 없음", username));
		}
		...
		// 문자열로 되어있는 권한 정보를 GrantedAuthority type 으로 변환 하기
		for(RoleVO r : roles) {
			grantList.add(new SimpleGrantedAuthority(r.getR_role()));
		}
		
		// DB에서 가져온 userVO, password 와 grantList token 발행하기
